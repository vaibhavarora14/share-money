...
const expectedAuthHost = process.env.EXPECTED_SUPABASE_AUTH_HOST ?? 'auth.sharedmoney.app';
...
function findSupabaseHost(bundleText) {
  const hosts = new Set();
  for (const match of bundleText.matchAll(/https:\/\/([a-z0-9-]+\.supabase\.co|[a-z0-9.-]*sharedmoney\.app)\b/gi)) {
    hosts.add(match[1].toLowerCase());
  }

  if (hosts.has(expectedAuthHost.toLowerCase())) {
...
if (deployedAuthHost && deployedAuthHost !== expectedAuthHost.toLowerCase()) {
...
if (deployedAuthHost) {
  ...
  if (!location) {
    ...
    if (callbackHost !== expectedAuthHost.toLowerCase()) {
