 ...
 * - POST /participants/:id/connect - Connect a person to an existing ShareMoney account by email
 * 
    ...
    if (httpMethod !== 'GET') {
      ...
      if (httpMethod === 'POST' && actionParticipantId && isValidUUID(actionParticipantId)) {
        ...
        if (action === 'connect') {
          ...
          if (!targetUserId) {
            return createErrorResponse(404, 'No ShareMoney account was found for this email yet', 'NOT_FOUND', undefined, req);
          }
