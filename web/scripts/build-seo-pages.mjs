...
const siteUrl = "https://owewho.com";
const appUrl = `${siteUrl}/app`;
const ogImage = `${siteUrl}/og-owewho.png`;

const pages = [
  {
    path: "/",
    title: "OweWho - Split Bills and Track Group Expenses",
    description:
      "OweWho helps friends, roommates, and travel groups split bills, track shared expenses, and see who owes who without handling payments.",
    keywords:
    ...
    body:
      "Keep one shared ledger for trips, roommates, dinner groups, and everyday shared costs. OweWho shows balances clearly while payments stay in your preferred payment app.",
    proof: ["Trip and roommate ledgers", "Currency-specific balances", "No bank link or payment handling"],
  },
  {
    path: "/split-bills",
    title: "Split Bills Online with Friends | OweWho",
    description:
  ...
  },
  {
    ...
    title: "Group Expense Tracker for Friends and Families | OweWho",
    description:
    ...
    body:
      "OweWho keeps group costs organized by member, currency, and activity so every person can see what changed and what remains open.",
    proof: ["Member balances", "Expense activity", "Shared group history"],
  },
  {
    path: "/trip-expense-splitter",
    title: "Trip Expense Splitter for Travel Groups | OweWho",
    description:
  ...
  },
  {
    ...
    title: "Roommate Expense Tracker for Rent and Utilities | OweWho",
    description:
  ...
  },
  {
    ...
    title: "Splitwise Alternative for Shared Expenses | OweWho",
    description:
      "Looking for a Splitwise alternative? OweWho gives groups a simple shared ledger, clear balances, and payment-free settlement tracking.",
    keywords:
    ...
    body:
      "OweWho focuses on the shared record: who paid, who joined, what remains open, and when a balance was settled outside the app.",
    proof: ["Splitwise CSV import", "No payment instruments stored", "Free shared expense tracking"],
  },
  {
    path: "/in/splitwise-alternative",
    title: "Splitwise Alternative India for Friends and Trips | OweWho",
    description:
      "OweWho is a Splitwise alternative for India-focused groups that track trip, roommate, and dinner expenses while settling outside the app.",
    keywords:
    ...
    body:
      "Track INR group expenses for trips, roommates, and dinner plans. When the balance is clear, settle in your preferred payment app and record it in OweWho.",
...
function fallbackMarkup(page) {
  ...
  return `<main id="main-content"><section><p>${escapeHtml(page.eyebrow)}</p><h1>${escapeHtml(
    page.heading,
  )}</h1><p>${escapeHtml(page.body)}</p><ul>${proofItems}</ul><p><a href="${appUrl}">Open the OweWho web app</a></p></section></main>`;
}
...
function renderPage(template, page) {
  ...
  let html = template
    ...
    .replace(/<meta property="og:image:alt" content=".*?" \/>/s, `<meta property="og:image:alt" content="OweWho group balances and shared expense ledger." />`)
    ...
    .replace(/<meta property="twitter:image:alt" content=".*?" \/>/s, `<meta property="twitter:image:alt" content="OweWho group balances and shared expense ledger." />`)
