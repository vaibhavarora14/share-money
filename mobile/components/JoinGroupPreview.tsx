...
import { getGroupInvitePreviewRPC } from "../hooks/useGroupInvitations";
...
/**
 * Shown to logged-out users who open a group invite link.
 * Displays a safe preview (group name + member count) and routes them to
 * sign-in; the token is kept in storage so they auto-join after auth.
 */
export const JoinGroupPreview: React.FC<JoinGroupPreviewProps> = ({
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      try {
        const info = await getGroupInvitePreviewRPC(token);
        if (
          info &&
          info.is_valid &&
          info.group_name &&
        ...
        logError(err, { context: "JoinGroupPreview fetchInfo" });
        setError(
        ...
        <Surface style={styles.card} elevation={2}>
          ...
          <View style={styles.infoBox}>
            <Text variant="bodyMedium" style={styles.infoText}>
              You've been invited to join this group. Sign in (or create an
