...
export const getAuthToken = async (session: Session | null, signOut: () => Promise<void>): Promise<string | null> => {
...
export const apiFetch = async (
  ...
  const token = await getAuthToken(session, signOut);
  if (!token) {
  ...
  if (!response.ok) {
    await handleApiError(response, signOut);
  }
